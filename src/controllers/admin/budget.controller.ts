...
export const adminDeleteBudget = async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const { reason } = req.body; // Admin xóa cũng cần lý do!

  try {
    // 1. Tìm ngân sách (kèm user để lấy info gửi thông báo)
    ...
    // 3. Gửi thông báo cho User (Rất quan trọng)
    ...
    await createAndSendNotification(
        budget.user._id, 
        "warning", // Dùng warning để gây chú ý
        message, 
        "/budget"
    );
    ...
    await logAction(req, {
      ...
      level: "critical",
      metadata: {
        deletedBudget: budget.toObject(), // Lưu lại bản sao lưu phòng khi xóa nhầm
        reason: reason
      }
    });
